...
# with command line options: step4 --data --filetype DQM --conditions 106X_dataRun2_v20 -s HARVESTING:@rerecoZeroBias --customise_commands=process.DQMStore.saveByLumi = cms.untracked.bool(True) --era Run2_2018 --scenario pp --filein file:RECO_RAW2DIGI_L1Reco_RECO_ALCA_EI_PAT_DQM_inDQM.root --python_filename=harvesting_Run2018A_ZeroBias.py --no_exec
...
# Schedule definition
process.schedule = cms.Schedule(process.DQMHarvestCommonSiStripZeroBias_step,process.DQMCertCommon_step,process.DQMHarvestMuon_step,process.DQMCertMuon_step,process.DQMHarvestHcal_step,process.DQMHarvestJetMET_step,process.DQMCertJetMET_step,process.DQMHarvestEcal_step,process.DQMCertEcal_step,process.DQMHarvestEGamma_step,process.DQMHarvestL1TMuon_step,process.DQMHarvestL1TEgamma_step,process.dqmsave_step)
